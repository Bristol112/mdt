    ...
    <body>
        ...
        <main role="main" class="container">
            <div class="content-introduction">
                ...
                <p class="lead">Here you can create a report.<br/>You can attach a civil service number to the report (which must have a profile) or you can leave it blank and add it later.<br/>You can also add fines (if needed) at the bottom of the page.</br>To remove a fine, click on the same fine in the "Selected Fines" section"“</p>
            </div>
            <div class="createreport-container">
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "edit" && !empty($selectedreport)) { ?>
                    <form method="post">
                        ...
                        <div class="input-group mb-3">
                            <input type="text" name="title" class="form-control login-user" value="<?php echo $selectedreport["title"]; ?>" placeholder="Fingerprint" required>
                        </div>
                        <?php if (!empty($profiledata)) { ?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="<?php echo $profiledata["citizenid"]; ?>" placeholder="CSN (find it in ID at first) (it might be empty)">
                            </div>
                        <?php } else {?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="" placeholder="CSN (find it in ID at first) (it might be empty)">
                            </div>
                        <?php } ?>
                        ...
                        <div class="form-group">
                            <button type="submit" name="create" class="btn btn-primary btn-police">Edit the report</button>
                        </div>
                    </form>
                <?php } else { ?>
                    <form method="post">
                        ...
                        <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "createnew") { ?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="<?php echo $selectedprofile["citizenid"]; ?>" placeholder="CSN (Find it in ID at first) (It might be blank)">
                            </div>
                        <?php } else {?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="" placeholder="CSN (Find it in ID at first) (It might be blank)">
                            </div>
                        <?php } ?>
                        <div class="input-group mb-2">
                        ...
                        <div class="form-group">
                            <button type="submit" name="create" class="btn btn-primary btn-police">Save the report</button>
                        </div>
                    </form>
                <?php } ?>
                </div>
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "edit" && !empty($selectedreport)) { ?>
                    <div class="createreport-right">
                        <h5> Provided penalties</h5>
                        <p class="total-punishment">Overall: $<?php echo $totalprice; ?> - <?php echo $totalmonths; ?> Month</p>
                        ...
                        <?php if (!empty($lawsedit_array)) { ?>
                            <?php foreach($lawsedit_array as $issalaw) { ?>
                                <div class="report-law-item" data-toggle="tooltip" data-html="true" title="<?php echo $issalaw["description"]; ?>">
                                    ...
                                    <p class="lawlist-fine">Fine: €<span class="fine-amount"><?php echo $issalaw["fine"]; ?></span></p>
                                    <p class="lawlist-months">The: <span class="months-amount"><?php echo $issalaw["months"]; ?></span> Month</p>
                                </div>
                            <?php } ?>
                        <?php } ?>
                        </div>
            ...
            <?php if ($_SERVER['REQUEST_METHOD'] != "POST" || $_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] != "edit") { ?>
                <button type="button" class="btn btn-primary btn-police" id="togglelaws" style="margin-bottom:2vh!important;">Show fines</button>
                <div class="laws">
                    ...
                    <?php foreach($laws_array as $law){?>
                        <div class="report-law-item-tab" data-toggle="tooltip" data-html="true" title="<?php echo $law['description']; ?>">
                            ...
                            <p class="lawlist-fine">Fines: $<span class="fine-amount"><?php echo $law['fine']; ?></span></p>
        ...
        <script src="assets/js/main.js"></script>
         ...
         <script>
            ...
            new FroalaEditor('textarea#froala-editor', {
                ...
                spellcheck: false,
                language: 'nl',
                placeholderText: "قصه عن التقرير",
                toolbarButtons: 	{
                    'moreText': {

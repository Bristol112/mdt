    ...
    <body>
        ...
        <main role="main" class="container">
            <div class="content-introduction">
                <h3>Reports</h3>
                <p class="lead">You can search, read, update and delete reports here. <br/> Please do not modify or delete any report without permission from your superior!</p>
            </div>
            <div class="profile-container">
                <div class="profile-search">
                    <?php if ($_SERVER['REQUEST_METHOD'] != "POST" || $_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] != "show") { ?>
                        <a href="createreport" class="btn btn-pol btn-md my-0 ml-sm-2">create a report</a>
                    <?php } else { ?>
                        <form method="post" action="createreport" class="d-inline">
                            <input type="hidden" name="type" value="edit">
                            <input type="hidden" name="reportid" value="<?php echo $selectedreport['id']; ?>">
                            <button type="submit" name="issabutn" class="btn btn-pol btn-md my-0 ml-sm-2">Modify report</button>
                        </form>
                        <form method="post" action="createreport" class="d-inline">
                            <input type="hidden" name="type" value="delete">
                            <input type="hidden" name="reportid" value="<?php echo $selectedreport['id']; ?>">
                            <?php if ($_SESSION["role"] == "admin") { ?>
                            <button type="submit" name="issabutn" class="btn btn-danger btn-md my-0 ml-sm-2">Create report</button>
                            <?php } ?>
                        </form>
                    <?php } ?>
                    <br /><br />
                    <form method="post" class="form-inline ml-auto">
                        ...
                        <div class="md-form my-0">
                            <input class="form-control" name="search" type="text" placeholder="look for reports" aria-label="Search">
                        </div>
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "search") { ?>
                    <div class="search-panel">
                        <h5 class="panel-container-title">Reports have been found.</h5>
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "show" && !empty($selectedreport)) { ?>
                    <div class="report-show">
                        ...
                        <?php if ($profiledata != NULL) {?>
                            <p>Regarding: <?php echo $profiledata["fullname"]; ?> (<?php echo $profiledata["citizenid"]; ?>)
                        <?php } ?>
                        <hr>
                        <strong>Message:</strong>
                        <p class="report-description"><?php echo $selectedreport["report"]; ?></p>
                        <p class="report-author"><i>Writer: <?php echo $selectedreport["author"]; ?></i></p>
                    </div>
                    <div class="laws-added">
                        <?php if ($lawdata != NULL) {?>
                            <h5 class="report-laws-title">A sentence has been issued:</h5>
                            <?php foreach($lawdata as $law){?>
                                <div class="law-item" data-toggle="tooltip" data-html="true" title="<?php echo $law['description']; ?>">
                                    ...
                                    <p class="lawlist-months">Time: <?php echo $law['months']; ?> Months</p>
                                </div>
                            <?php }?>
                        <?php } ?>
                    </div>

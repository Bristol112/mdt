<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        if ($_POST['type'] == "search") {
            $result = $con2->query("SELECT * FROM player_houses WHERE concat(' ', citizenid, ' ') LIKE '%".$con->real_escape_string($_POST['search'])."%'");
            $search_array = [];
            ...
            $query = $con2->query("SELECT * FROM player_houses WHERE id = ".$con->real_escape_string($houseid));
    ...
    <body>
        ...
        <main role="main" class="container">
            <div class="content-introduction">
                <h3>Houses</h3>
                <p class="lead">Here you can search for a house and see who owns it.<br/>Thanks to this, it is possible to visit people's houses..</p>
            </div>
            <div class="profile-container">
                <div class="profile-search">
                    <form method="post" class="form-inline ml-auto">
                        ...
                        <div class="md-form my-0">
                            <input class="form-control" name="search" type="text" placeholder="House name" aria-label="Search">
                        </div>
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "search") { ?>
                    <div class="search-panel">
                        <h5 class="panel-container-title">Found houses..</h5>
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "show" && !empty($selectedHouse)) { ?>
                    <div class="profile-panel">
                        <div class="profile-information">
                            <p><strong>House name:</strong><br><?php echo $selectedHouseInformation["label"]; ?></p>
                            <p><strong>Key holder:</strong><br>
                                <?php
                                    ...
                                    $keyholders_data = $con2->query("SELECT * FROM players WHERE ".implode(" OR ", $columns));
                                    $keyholders_array = [];
                            ...
                            ?>
                            <p><strong>House owner:</strong><br><?php echo $firstname_owner . " " . $lastname_owner ?></p>
                            <p><strong>Owner CSN:</strong><br><?php echo $selectedHouse["citizenid"]; ?></p>
                            <p><strong>House location:</strong><br><?php echo 'x: ' . $enter_coords['x'] . '<br>y: ' . $enter_coords['y'] . '<br>z: ' . $enter_coords['z']?></p>
                            <p>
                                <strong>Garage:</strong><br>
                                <?php if(trim($selectedHouseInformation['garage']) == null) echo '<span class="text-danger">لا يوجد</span>' ?>
                                <?php if(trim($selectedHouseInformation['garage']) == !null) echo '<span class="text-success">يوجد</span>' ?>
                            </p>
                        </div>
                    </div>
                    <div class="profile-panel">
                        <div class="profile-information">
                            <form method="POST" action="setgps.php?locx=<?php echo $enter_coords['x']?>&locy=<?php echo $enter_coords['y']?>&locz=<?php echo $enter_coords['z']?>">
                                <div class="form-group">
                                    <label><strong>Steam name</strong></label>
                                    <input type="text" class="form-control" placeholder="Steam" name="steamname">
                                </div>
                                <button type="submit" name="issabutn" class="btn btn-pol btn-md">GPS Location</button>
                            </form>
                        </div>
                    </div>
                <?php } ?>
